...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Pull stored progress
    try {
      const progressRaw = window.localStorage.getItem("gft-progress");
      if (progressRaw) {
        const parsed = JSON.parse(progressRaw);
        const workouts = parsed.workouts ?? [];
        streak = workouts.length;
        lastWorkout = workouts.at(-1) ?? null;
      }
    } catch {
      // ignore parse errors
    }

    // Get time from AISense public Time API (timezone-aware)
    const loadTime = async () => {
      try {
        const offset = formatOffset(); // e.g. +1300 for NZDT
        const res = await fetch(`https://aisenseapi.com/services/v1/datetime/${offset}`);
        const data = await res.json();
        const iso = data?.datetime;
        if (iso) {
          const current = new Date(iso);
          nextMealLabel = pickMeal(current);
        }
      } catch {
        // fallback to local clock if API unavailable
  ...
  // const highlights = [
  //   { label: "Beginner safe", value: "Gentle strength + walks" },
  //   { label: "Meals ready", value: mealSuggestion || "4 low-sugar plates/day" },
  //   { label: "Streak saver", value: `${streak} day streak logged` },
  // ];

  const dashWorkout = lastWorkout ? `${lastWorkout.difficulty ?? "easy"} day` : "No workout logged yet";
  const dashWorkoutSub = lastWorkout ? `Last done: ${lastWorkout.label}` : "Log a workout to start your streak";
        ...
        <section className="grid lg:grid-cols-2 gap-12 items-center pt-14">
          <div className="space-y-7">
            ...
            <p className="text-lg text-muted max-w-2xl">
              GlowUp keeps you organised while you plan workouts, plate up yummy South Indian meals, and celebrate the
              small wins.
            </p>
            ...
            {/* <div className="grid sm:grid-cols-3 gap-3">
              {highlights.map((item) => (
                <div
                  key={item.label}
                  className="rounded-2xl bg-white/80 border border-white/60 px-4 py-3 text-sm text-ink shadow-inner"
                >
                  <p className="uppercase tracking-[0.14em] text-xs text-muted font-semibold">{item.label}</p>
          ...
          >
            <div className="absolute -right-14 -top-14 w-60 h-60 rounded-full bg-lavender/70 halo" aria-hidden />
            <div className="absolute -left-10 -bottom-12 w-52 h-52 rounded-full bg-pink/70 blur-3xl" aria-hidden />
            <SectionHeader eyebrow="Daily dashboard" title="Your glow-up snapshot" subtitle="Quick hits to stay on track" />
            <div className="mt-6 space-y-4 text-sm">
              <div className="rounded-2xl bg-gradient-to-br from-pink/80 to-lavender/80 p-4 text-ink soft-shadow">
                <p className="text-xs uppercase tracking-[0.12em] font-semibold">Workout</p>
              ...
              <div className="rounded-2xl bg-white p-4 text-ink border border-white/70 shadow-inner shadow-peach/20">
                <p className="text-xs uppercase tracking-[0.12em] font-semibold">Meal</p>
              ...
              <div className="rounded-2xl bg-sage/80 p-4 border border-white/70">
                <p className="text-xs uppercase tracking-[0.12em] font-semibold">Streak</p>
              ...
              <Link key={card.title} href={card.link} className="block group ">
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="rounded-full bg-ink text-cream px-3 py-1 text-xs font-semibold">{card.badge}</span>
                    <span className="rounded-full bg-pink/50 px-3 py-1 text-[11px] font-semibold text-ink">Glow vibe</span>
                  </div>
                  ...
                  {/* <span className="text-sm font-semibold text-peach ">Open -{'>'}</span> */}
                </motion.div>
              </Link>
            ))}
          </div>
        ...
        <section className="mt-14">
          <div className="rounded-[32px] bg-white/90 border border-white/70 card-shadow p-7 sm:p-9 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.14em] font-semibold text-muted">Daily rhythm</p>
              <h3 className="text-2xl font-semibold text-ink">Warm body, balanced plate, gentle close</h3>
              <p className="text-sm text-muted max-w-xl">
                Open with breath, move with intention, refuel with steady carbs and protein, close with a quick walk.
              </p>
            </div>
            <div className="flex flex-wrap gap-3 text-sm">
              <span className="rounded-full bg-pink/60 px-4 py-2 font-semibold text-ink">Mobility + strength</span>
              <span className="rounded-full bg-lavender/70 px-4 py-2 font-semibold text-ink">Low-sugar South Indian meals</span>
              <span className="rounded-full bg-sage/80 px-4 py-2 font-semibold text-ink">10-12 min walk</span>
            </div>
          </div>
        </section>
      </main>
